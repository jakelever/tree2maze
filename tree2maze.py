...
if __name__ == '__main__':
	...
	for layer in range(1,6):
		...
		for name,chain in chains.items():
			...
			for (x1,y1),(x2,y2) in zip(chain[:-1],chain[1:]):
				...
				segments.append((x1,y1,x2,y2,name))
	...
	minX = min ( min(x1,x2) for x1,y1,x2,y2,_ in segments )
	minY = min ( min(y1,y2) for x1,y1,x2,y2,_ in segments )
	maxY = max ( max(y1,y2) for x1,y1,x2,y2,_ in segments )
	...
	#for layer:wq in range(1):
	#	for (startX,startY,startDir) in turtles:
	dwg.save()
